                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        @if($student->isActive())
                                        <button type="button" class="btn btn-success" onclick="showPromoteModal({{ $student->id }}, '{{ $student->full_name }}', {{ $student->class_id ?? 'null' }})"><i class="fas fa-arrow-up"></i></button>
                                        <button type="button" class="btn btn-danger" onclick="showDemoteModal({{ $student->id }}, '{{ $student->full_name }}', {{ $student->class_id ?? 'null' }})"><i class="fas fa-arrow-down"></i></button>
                                        ...
                                        @endif
                                        <button type="button" class="btn btn-danger" onclick="confirmDelete({{ $student->id }}, '{{ $student->full_name }}')"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
...
function showModal(id) { const m=document.getElementById(id); if(m){const b=new bootstrap.Modal(m); b.show();}}
function showPromoteModal(id, name, classId) { document.getElementById('promoteStudentId').value=id; document.getElementById('promoteFromClassId').value=classId; document.getElementById('promoteStudentName').textContent=name; document.getElementById('promoteForm').action="{{ route('admin.students.bulk-promote') }}"; showModal('promoteModal'); }
function showDemoteModal(id, name, classId) { document.getElementById('demoteStudentId').value=id; document.getElementById('demoteFromClassId').value=classId; document.getElementById('demoteStudentName').textContent=name; document.getElementById('demoteForm').action="{{ route('admin.students.bulk-demote') }}"; showModal('demoteModal'); }
function showGraduateModal(id, name) { document.getElementById('graduateStudentId').value=id; document.getElementById('graduateStudentName').textContent=name; document.getElementById('graduateForm').action="{{ route('admin.students.bulk-graduate') }}"; showModal('graduateModal'); }
...
function showBulkPromoteModal() { const s=getSelectedStudentIds(); if(!s.length){alert('Select students');return;} document.getElementById('bulkPromoteStudentIds').innerHTML=s.map(i=>'<input type="hidden" name="student_ids[]" value="'+i+'">').join(''); showModal('bulkPromoteModal'); }
function showBulkDemoteModal() { const s=getSelectedStudentIds(); if(!s.length){alert('Select students');return;} document.getElementById('bulkDemoteStudentIds').innerHTML=s.map(i=>'<input type="hidden" name="student_ids[]" value="'+i+'">').join(''); showModal('bulkDemoteModal'); }
function showBulkGraduateModal() { const s=getSelectedStudentIds(); if(!s.length){alert('Select students');return;} document.getElementById('bulkGraduateStudentIds').innerHTML=s.map(i=>'<input type="hidden" name="student_ids[]" value="'+i+'">').join(''); showModal('bulkGraduateModal'); }
</script>
@endsection
